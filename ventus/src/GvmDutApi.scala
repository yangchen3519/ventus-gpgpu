...
class GvmDutVReg extends ExtModule with HasExtModuleInline {
  val io = IO(new Bundle {
    val clock          = Input(Clock())
    val sm_id          = Input(UInt(32.W))
    val vbanks         = Input(UInt((NUMBER_VGPR_SLOTS * num_thread * 32).W))
  })

  setInline("GvmDutVReg.sv",
    s"""
    |// generated by Chisel ExtModule
    |module GvmDutVReg (
    |  input  wire               io_clock,
    |  input  wire [31:0]        io_sm_id,
    |  input  wire [${(NUMBER_VGPR_SLOTS * num_thread * 32) - 1}:0] io_vbanks
    |);
    |
    |  import "DPI-C" function void c_GvmDutVReg(
    |    input int num_sm,
    |    input int sm_id,
    |    input int num_bank,
    |    input int num_vgpr_slots,
    |    input int num_thread,
    |    input int vbanks_word,
    |    input int vbanks_word_idx,
    |    input int thread_idx
    |  );
    |
    |  integer i, j;
    |  always @(posedge io_clock) begin
    |    // call DPI-C func every cycle
    |    for (i = 0; i < ${NUMBER_VGPR_SLOTS}; i = i + 1) begin
    ...
    |          ${NUMBER_VGPR_SLOTS},
    |          ${num_thread},
    |          io_vbanks[(32*${num_thread}*i + 32*j) +:32],
    |          i,
    |          j
    |        );
    |      end
