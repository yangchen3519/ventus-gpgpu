...
> 状态：**已实现：A/B/C + L1D Cache 计数 + 程序级（首 kernel 到末 kernel）启停/清零/汇总**；**待实现：D/E/F/G/H/I**  
> 最近更新：2026-04-12  
...
- 输出粒度：**按程序窗口输出**（首个 kernel 第一次 dispatch 时清零，最后一个 kernel 全部结束后 dump）
...
- `top/GPGPU_top.scala`：已做多 SM 汇总、启停与程序级打印状态机
